...
class ChatViewModel : ViewModel() {
    ...
    // Cache user info to avoid redundant API calls
    ...
    init {
        ...
        viewModelScope.launch { currentUserId = tokenManager.getUserIdSync() }

        // Listen for incoming chat messages from WebSocket
        viewModelScope.launch {
            webSocketManager.chatMessages.collect { wsMessage -> handleIncomingMessage(wsMessage) }
        }
    }

    /** Connect to WebSocket */
    fun connectWebSocket() {
        viewModelScope.launch { webSocketManager.connect() }
    }
                ...
                if (response.code == "200" && response.result != null) {
                    ...
                    val userFetchJobs =
                            allParticipantIds.map { participantId ->
                                async {
                                    try {
                                        val userResponse = userApi.getUserById(participantId)
                                        if (userResponse.code == "200" &&
                                                        userResponse.result != null
                                        ) {
                                            userCache[participantId] = userResponse.result
                                        }
                                    } catch (e: Exception) {
                    ...
                    userFetchJobs.awaitAll()

                    // Enrich conversations with participant names and avatars
                    val enrichedConversations =
                            conversations.map { conv ->
                                ...
                                conv.participantIds.forEach { participantId ->
                                    userCache[participantId]?.let { user ->
                                        participantNames[participantId] = user.fullName
                                        user.avatarUrl?.let { avatarUrl ->
                                            participantAvatars[participantId] = avatarUrl
                                        }
                                    }
                                }

                                conv.copy(
                                        participantNames = participantNames.ifEmpty { null },
                                        participantAvatars = participantAvatars.ifEmpty { null }
                                )
                            }

                    _conversationsState.update {
                        it.copy(conversations = enrichedConversations, isLoading = false)
                    }
                } else {
                    _conversationsState.update {
    ...
    fun loadMessages(conversationId: String, loadMore: Boolean = false) {
        ...
        viewModelScope.launch {
            ...
            _chatDetailState.update { it.copy(isLoading = true, error = null, currentPage = page) }
            ...
            try {
                ...
                if (response.code == "200" && response.result != null) {
                    ...
                    _chatDetailState.update { it.copy(isLoading = false, error = response.message) }
                }
            } catch (e: Exception) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (response.code == "200" && response.result != null) {
                    ...
                    _chatDetailState.update { state ->
                        state.copy(messages = state.messages + sentMessage, isSending = false)
                    }
                    ...
                    _chatDetailState.update { it.copy(isSending = false, error = response.message) }
                }
            } catch (e: Exception) {
                _chatDetailState.update {
                            ...
                            conversations =
                                    state.conversations.map { conv ->
                                        if (conv.id == conversationId) {
                                            val updatedUnreadCounts =
                                                    conv.unreadCounts?.toMutableMap()?.apply {
                    ...
                    conversations =
                            state.conversations.map { conv ->
                                if (conv.id == conversationId) {
                                    val updatedUnreadCounts =
                                            conv.unreadCounts?.toMutableMap()?.apply {
                                                currentUserId?.let { put(it, (get(it) ?: 0) + 1) }
                                            }
                                    conv.copy(unreadCounts = updatedUnreadCounts)
                                } else {
                                    conv
    ...
    fun setCurrentConversation(conversation: ConversationResponse) {
        ...
        _chatDetailState.update { ChatDetailState(conversation = conversation) }
    }
